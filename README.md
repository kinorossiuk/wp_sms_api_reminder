...
- **임시 메모**: 현재 브라우저에만 짧은 메모를 저장하고, 다음 날 자정(00:00 KST)에 자동 삭제합니다.
- **집중 타이머**: 시간·분·초 또는 빠른 시간 설정으로 실행하며, 종료 시 화면·소리·OS 알림으로 알려줍니다.

## 라이선스

이 프로젝트는 [MIT License](LICENSE)로 배포됩니다. 저작권 및 라이선스 고지를
포함하는 조건으로 사용, 복사, 수정, 배포 및 상업적 이용을 허용하며, 소프트웨어는
보증 없이 제공됩니다.

포함된 제3자 구성 요소의 저작권과 라이선스 고지는
[`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md)에 별도로 유지합니다.
...
- 직접 입력한 문자의 화면 글자 수(복합 이모지 포함), Unicode 코드 포인트 수, UTF-16 문자열 길이, UTF-8 바이트 수를 실시간 확인
- 현재 기기의 기본 이모지를 단일 코드 포인트·복합 조합으로 구분해 검색·선택하고 최대 20개 무작위 생성
- 499·500·501자 등 화면 글자 수 경계값 생성·확인·복사
...
`/tools/temporary-note/`에서 잠깐 보관할 메모를 작성합니다.

- 메모는 현재 브라우저의 로컬 저장 공간에만 보관하고 서버로 전송하지 않음
- 최대 5,000자까지 작성 가능
- 다음 날 00:00 KST에 자동 삭제되며 삭제된 메모는 복구할 수 없음
- 다른 기기나 브라우저에서는 확인할 수 없음

## 집중 타이머

`/tools/timer/`에서 현재 브라우저에서만 실행하는 타이머를 사용합니다.

- 시간·분·초 단위 입력 및 1·5·10·25분 빠른 설정
- 시작, 일시정지, 초기화 지원
- 종료 시 화면 강조와 알림음 제공
- 알림 권한을 허용하면 OS 알림 제공

페이지나 브라우저를 완전히 닫거나 기기가 절전 상태이면 종료 알림이 지연되거나
울리지 않을 수 있습니다.
